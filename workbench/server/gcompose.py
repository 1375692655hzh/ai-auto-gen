...
── 爆款技巧包(2026-09-11 双素材 A/B 实测(kimi-k3 四稿对照)确认做进; 优先级高于上述通用铁律, 但零观点类模板的禁观点/禁情绪约束仍优先于本包) ──
- 钩子公式(首行三选一): ①反常识断言(挑战读者默认认知的判断) ②最大数字冲击(全帖最强的那个数字+对比锚, 砸在第一行) ③身份代入("如果你在用 X, 你的…已经…")
- 钩子数字只能是素材原文出现的数字; 跨条目加总须显式标"合计约"且可复算, 禁止无中生有
- 折叠线以上的前 3 行决定 80% 的传播: hook 行 + 1 行定调 + 1 行悬念(为什么/接下来会怎样), 不许做任何铺垫
- 数字具体化: 删掉"大量/明显/严重/很多"这类模糊量词, 每个论点必须挂数字或对比锚(写"1.51 亿次", 不写"次数惊人")
- 节奏: 动词开头的短句; 关键转折词独立成行(但/结果/问题是); 并列清单用 ①②③ 编号制造信息密度感
- 情绪人设: 全帖恰好处放一处作者真实反应(一句吐槽/一声感叹, 如"把我看郁闷了"), 让读者觉得是人在说话不是机器在播报; 只许一处, 禁煽情堆砌
- 金句位: 倒数第二行放一句可被单独截图转发的浓缩判断(30 字内, 用对仗或反差结构)
- 结尾钩: 留一个未答之问或未完之事, 用具体的下一步悬念(下一份报告/下一个是谁/明天验证什么), 禁硬凹「你怎么看?」「欢迎讨论」"""
