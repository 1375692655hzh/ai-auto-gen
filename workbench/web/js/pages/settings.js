  ...
  <div style="max-width:760px">
    ...
    <div class="card" v-show="sec==='translate'">
      ...
      <div class="key-guide">
        ...
        把出口切到非封锁地区即可(美国/新加坡实测可用, 日本/欧洲大概率可用)。<br>
        <b>⚠ 只开「系统代理」开关没用:</b> 未配「账号代理」时 OmniRoute 免费位强制直连,
        系统代理/环境代理都不影响它——只有下面两条路能改它的出口。<br>
        <b>【方式 A · Clash Verge 开 TUN(最省事, 全局接管)】</b><br>
        ① 打开 Clash Verge, 在节点列表挑一个<b>非封锁地区</b>节点
        (美国/新加坡/日本等; 避开香港/中国大陆);<br>
        ② 打开「TUN 模式」开关(首页或 设置→系统设置 里; 首次开启会请求
        管理员权限/安装服务, 同意即可);<br>
        ③ 回到工作台刷新一条英文推文, 出译文即通。
        代价: TUN 开着期间整机流量都走代理, 不想影响其它应用就看完关掉、改用方式 B。<br>
        <b>【方式 B · 只代理 OmniRoute 自己(不动全局, 一次性配置, 推荐)】</b><br>
        ① Clash Verge 保持运行, 到「设置」页记下「混合端口」(默认 7897),
        节点选非封锁地区; <b>不用开 TUN、不用开系统代理</b>;<br>
        ② 浏览器打开 OmniRoute 面板 <code>http://127.0.0.1:20128</code> →
        找到 opencode 免费账号的账号卡 → 「账号代理」: 类型 HTTP、
        地址 127.0.0.1、端口填第①步记下的 → 保存;<br>
        ③ 回本页刷新一条英文推文验证出译文。配置持久保存, 一次配好长期有效,
        系统其它流量完全不受影响。<br>
        <b>一个坑:</b> 配了账号代理后 Clash 若完全退出, 免费位会 503 快速失败
        (不会偷偷回落大陆出口撞 403)——链自动落 mimo, 重开 Clash 即恢复。<br>
        <b>没装代理软件?</b> 任意常见客户端(Clash Verge / v2rayN 等)均可, 步骤同上;
        实在不想折腾代理 → 直接用下方 DeepSeek 付费位(大陆直连零配置)。<br>
        <b>备选方案(付费兜底, 可选):</b>
    ...
    <div class="card" v-show="sec==='video'">
      ...
      <div class="key-guide">
        ...
        <b>环境依赖:</b> 测试/合成在本机跑 Node 脚本——需 <b>Node.js ≥ 20</b>（<a href="https://nodejs.org" target="_blank" rel="noopener">nodejs.org</a> 装 LTS 版）
        + 仓库带 <code>ai-workflow/video/</code> 目录并在其中执行过 <code>npm install</code>（完整克隆后跑一次 <code>bin/setup-workbench.cmd</code> 即全配好）；只拷 workbench 部分目录的机器配音/出片不可用。报错会给具体补救指引。
      </div>
