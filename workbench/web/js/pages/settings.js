  ...
  <div style="max-width:760px">
    ...
    <div class="card" v-show="sec==='translate'">
      ...
      <div class="key-guide">
        ...
        <b>地区封锁真相(2026-09-11 源码审计修正):</b> 免费位直连 opencode.ai 公共端点,
        封锁是 opencode.ai 的<b>模型级门禁</b>——只封 muse(大陆/香港出口被拒;
        美国/新加坡实测可用, 日本/欧洲大概率可用), <b>mimo 无地区门禁, 大陆裸网也能用</b>
        ——所以 VPN 随便切, 链会自动落到 mimo, 不会全断。<br>
        另: 未配「账号代理」时 opencode 位强制直连(系统代理/环境代理都无效),
        想让出口走 Clash 只有两条路:<br>
        <b>【方式 A · 开 TUN(最省事)】</b> Clash Verge 开 TUN 模式 + 任意非封锁地区节点
        (美/新/日/欧均可), 网络层全接管, 免费位即装即用; 代价是全局流量都过代理。<br>
        <b>【方式 B · 不开 TUN, 只代理 OmniRoute 自己(一次性配置)】</b>
        机制(源码确认): opencode 免费账号卡可挂独立代理, 整条"本机→opencode.ai"链路
        走 undici ProxyAgent 隧道, 无泄漏, 系统其它流量完全不受影响, 与 TUN 开关解耦。步骤:<br>
        ① Clash 保持运行, 确认 HTTP 混合端口(Verge 默认 7897)在监听;
           节点选<b>任意非封锁地区</b>(美国/新加坡已实证)。<br>
        ② 浏览器打开 OmniRoute 面板 <code>http://127.0.0.1:20128</code> →
           找到 opencode 免费账号的账号卡 → 「账号代理」: 类型 HTTP、
           地址 127.0.0.1、端口填第①步端口 → 保存。<br>
        ③ 回本页刷新一条英文推文验证出译文; 此后 TUN 可关。
           配置持久保存, 一次配好长期有效。<br>
        <b>一个坑:</b> 配了账号代理后 Clash 若完全退出, 免费位会 503 快速失败
        (不会偷偷回落大陆出口撞 403)——链自动落 mimo/GLM, 重开 Clash 即恢复。<br>
        <b>备选方案(付费兜底, 可选):</b>
