...
@source("jin10_flash", "flash", "金十数据快讯", ttl_min=10, default_enabled=True)
def _jin10(conf):
    return gs.fetch_jin10_flash(int(conf.get("page_size", 50)))


@source("fxstreet_flash", "flash", "FXStreet外汇央行快讯",
        ttl_min=10, default_enabled=False)
def _fxstreet(conf):
    return gs.fetch_fxstreet_flash(int(conf.get("page_size", 30)))


@source("investinglive_flash", "flash", "investingLive外汇/美股/宏观快讯",
...
@source("cls_morning", "peer_article", "财联社有声早报", ttl_min=120)
def _cls(conf):
...
@source("cnbc_morning", "peer_article", "CNBC Daily Open美股晨报", ttl_min=120)
def _cnbc(conf):
    r = ges.fetch_cnbc_morning()
    return [r] if r else []


@source("japan_morning", "peer_article", "共同社日本市场精选", ttl_min=60)
def _japan(conf):
    r = ges.fetch_japan_morning()
    return [r] if r else []


@source("korea_morning", "peer_article", "韩联社韩国市场精选", ttl_min=60)
def _korea(conf):
    r = ges.fetch_korea_morning()
    return [r] if r else []


@source("em_research", "peer_article", "东财研报中心机构观点索引", ttl_min=120)
def _emr(conf):
    r = ges.fetch_em_research()
    return [r] if r else []


@source("sina_vip", "peer_article", "新浪意见领袖", ttl_min=60)
def _svip(conf):
    r = ges.fetch_sina_vip()
    return [r] if r else []


@source("cnyes_tw", "peer_article", "鉅亨网台股精选", ttl_min=60)
def _cnyes(conf):
...
@source("etnet_open", "peer_article", "etnet經濟通開市Go港股晨报", ttl_min=120)
def _etnet(conf):
    r = ges.fetch_etnet_open()
    return [r] if r else []


@source("newsquawk_open", "peer_article", "Newsquawk欧美市场开盘综述", ttl_min=120)
def _nq(conf):
    r = ges.fetch_newsquawk_open()
    return [r] if r else []


@source("smm_metals", "peer_article", "SMM上海有色网大宗商品日报", ttl_min=120)
def _smm(conf):
    r = ges.fetch_smm_metals()
    return [r] if r else []


@source("gangtise", "peer_article", "Gangtise投研日报", risk="high", ttl_min=120)
def _gangtise(conf):
...
@source("calendar", "calendar", "财经日历", ttl_min=120)
def _calendar(conf):
...
@source("peer_mornings", "peer_group", "同行早报聚合", ttl_min=120)
def _peers(conf):
...
@source("extras", "extras_group", "版式素材聚合", ttl_min=60)
def _extras(conf):
...
@source("miningcom", "peer_article", "MINING.COM大宗矿业新闻机器汇总",
        ttl_min=120, default_enabled=True)
def _mining(conf):
...
@source("liberty_street", "peer_article", "纽约联储Liberty Street分析博客",
        ttl_min=720, default_enabled=True)
def _liberty(conf):
...
@source("ths_flash", "flash", "同花顺全球财经直播快讯",
        ttl_min=10, default_enabled=True)
def _ths(conf):
    return gs.fetch_ths_flash(int(conf.get("page_size", 20)))


@source("caixin_flash", "flash", "财新数据通快讯",
        ttl_min=10, default_enabled=True)
def _caixin(conf):
    return gs.fetch_caixin_flash(int(conf.get("page_size", 30)))


@source("yahoo_headlines", "flash", "Yahoo财经大盘头条RSS",
        ttl_min=30, default_enabled=True)
def _yahoo(conf):
...
@source("sec_insider", "announcement", "SEC Form4内部人交易申报",
        ttl_min=30, default_enabled=True)
def _insider(conf):
    return gs.fetch_sec_edgar_filings(int(conf.get("page_size", 30)), form_type="4")


@source("cctv_xwlb", "peer_article", "央视新闻联播当日文字稿",
        ttl_min=720, default_enabled=True)
def _cctv(conf):
...
@source("polymarket_sentiment", "market", "Polymarket预测市场财经情绪",
        ttl_min=120, default_enabled=True)
def _polymarket(conf):
    return gs.fetch_polymarket_sentiment(int(conf.get("page_size", 10)))


@source("stocktwits_stream", "flash", "StockTwits美股情绪流",
...
@source("reddit_hot", "flash", "Reddit财经sub热帖",
...
@source("cn_index_snapshot", "market", "A股指数快照",
        ttl_min=15, default_enabled=True)
def _cn_idx(conf):
...
@source("em_sector_board", "market", "东财板块涨跌榜",
        ttl_min=60, default_enabled=True)
def _sector(conf):
