...
> 发布板块 2026-09-07 起拆为独立仓 **ai-auto-publisher**（私有，
> https://github.com/1375692655hzh/ai-auto-publisher）。克隆为本仓兄弟目录
> `../ai-auto-publisher`（或设 `AAG_AUTOPUB_ROOT`）后，下列 `python cli.py publish ...`
> 命令全部自动透传恢复；未就位时命令会提示并 exit 4。

1. **前置检查**：`python cli.py doctor`
   - Chrome 调试口(9222) 必须 ✅（让用户双击发布仓 `autopub/chrome_debug.bat`）。
...
红线：
- 不手编发布仓 `autopub/state.json`（防重发账本）。
